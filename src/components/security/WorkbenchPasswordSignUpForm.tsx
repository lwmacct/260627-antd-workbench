...
export function WorkbenchPasswordSignUpForm({ actions, challenge: challengeConfig, className, createImageChallenge, description, error, extraFields, loading = false, renderRemoteChallenge, terms, title, onSubmit }: WorkbenchPasswordSignUpFormProps) {
  ...
  return <div className={cx("wb-security-form", className)}>
    ...
    <Form<SignUpFormValues> clearOnDestroy form={form} layout="vertical" requiredMark={false} onFinish={(values: SignUpFormValues) => void submit(values)}>
      <Form.Item label={messages.password.username} name="username" rules={[{ required: true, message: messages.password.usernameRequired }]}><Input autoComplete="username" disabled={loading} prefix={<UserOutlined />} size="large" /></Form.Item>
      <Form.Item label={messages.password.password} name="password" rules={[{ required: true, message: messages.password.passwordRequired }, { min: 8, message: messages.password.passwordMinLength }]}><Input.Password autoComplete="new-password" disabled={loading} prefix={<LockOutlined />} size="large" /></Form.Item>
      <Form.Item dependencies={["password"]} label={messages.password.confirmPassword} name="confirmPassword" rules={[{ required: true, message: messages.password.confirmPasswordRequired }, ({ getFieldValue }) => ({ validator(_, value) { return !value || getFieldValue("password") === value ? Promise.resolve() : Promise.reject(new Error(messages.password.confirmPasswordMismatch)); } })]}><Input.Password autoComplete="new-password" disabled={loading} prefix={<LockOutlined />} size="large" /></Form.Item>
      {extraFields}
      ...
      <Button block disabled={Boolean(challengeConfig) && !challenge} htmlType="submit" loading={loading} size="large" type="primary">{messages.passwordSignUp.submit}</Button>
    </Form>
